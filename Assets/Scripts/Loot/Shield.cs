...
public class Shield : Loot {
	...
	void OnTriggerEnter2D(Collider2D collision)
	{
		...
		)
		{
			...
			if(tempPlayer.shieldDurability > 0)
			{
				tempPlayer.shieldDurability += 3;
			...
			{
				lifeTime = 0;	
				tempPlayer.go_player_shield_child.SetActive(true); // here we aativate the Player's  shield object
				tempPlayer.shieldDurability = 3;
				tempPlayer.frozenDurability = 0;
				gameManagerRef.loot.Remove(this.gameObject);
				Destroy(this.gameObject);
			}
